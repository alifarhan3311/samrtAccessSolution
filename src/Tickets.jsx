                    ...
                    <tr key={t._id}>
                      ...
                      <td>
                        {isAdmin && !['Resolved', 'Closed'].includes(t.status) ? (
                          <div className="ticket-agent-assigner">
                            <select 
                              className="assign-agent-styled-select"
                              value={t.assignedTo?._id || ''}
                              onChange={async (e) => {
                                const newAgentId = e.target.value;
                                const newAgent = agents.find(a => a._id === newAgentId) || null;
                                // Optimistic immediate state update
                                setTickets(prev => prev.map(tick => tick._id === t._id ? { ...tick, assignedTo: newAgent } : tick));
                                try {
                                  ...
                                  alert(err.message); 
                                  loadTickets(page); // revert if failed
                                }
                              }}
                            >
                              <option value="">-- Unassigned --</option>
                          ...
                          <div className="assigned-agent-display">
                            {t.assignedTo ? (
                              <span 
                                className="assigned-agent-name" 
                                style={['Resolved', 'Closed'].includes(t.status) ? { opacity: 0.85, background: '#f1f5f9', color: '#475569', borderColor: '#cbd5e1' } : {}}
                                title={['Resolved', 'Closed'].includes(t.status) ? `${t.status} tickets cannot be reassigned` : ''}
                              >
                                {t.assignedTo.name} {['Resolved', 'Closed'].includes(t.status) ? '🔒' : ''}
                              </span>
                            ) : (
