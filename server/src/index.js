...
app.get('/api/users/agents', auth, permit('admin', 'manager', 'agents'), async (req, res, next) => {
  try {
    const showAll = req.query.all === '1' && req.user.role === 'admin';
    const filter = showAll ? { role: 'agent' } : { role: 'agent', active: true };
    const agents = await User.find(filter).select('name email phoneNumber profilePicture active allowedTabs').sort({ name: 1 }).lean();
    const matchQuery = { status: { $nin: ['approved', 'cancelled'] } };
    if (req.query.date) {
      ...
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
        ...
        matchQuery.dueAt = { $gte: start, $lt: end };
      }
    }
    const workloads = await AgentJob.aggregate([{ $match: matchQuery }, { $group: { _id: '$agent', openJobs: { $sum: 1 } } }]);
    const counts = new Map(workloads.map(w => [String(w._id), w.openJobs]));
    res.json(agents.map(a => ({ ...a, available: true, openJobs: counts.get(String(a._id)) || 0 })));
  } catch (e) { next(e) }
});
...
app.get('/api/dashboard', auth, async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      cash: {
        today: {
          withdrawn: todayWithdrawals.total, dispatched: todayDispatched.total, actualLoaded: todayActualLoaded, returned: todayReturned.total,
          previousCash, vaultAvailable, balance: todayDispatched.total - todayReturned.total
        },
        month: {
          withdrawn: monthWithdrawals.total, dispatched: monthDispatched.total, actualLoaded: monthActualLoaded, returned: monthReturned.total,
...
app.post('/api/terminals/:id/assign', auth, permit('admin', 'manager', 'assign'), async (req, res, next) => {
  try {
    ...
    let agent;
    if (b.agentId) {
      if (t.official?.status === 'Inactive') return res.status(400).json({ message: `Cannot assign agent: Terminal ${t.terminalId} is currently Inactive. Please activate the terminal first.` });
      agent = await User.findOne({ _id: b.agentId, role: 'agent', active: true });
      if (!agent) return res.status(400).json({ message: 'Selected agent is unavailable' });
      if (b.cashToLoad == null || !b.dueAt) return res.status(400).json({ message: 'Cash to load and due time are required when assigning an agent' });
    }
    const now = new Date();
    const last = t.assignmentHistory.at(-1);
    const lastRecordedBiz = last ? last.businessName : (t.original?.businessName || t.official?.name || t.official?.tempName || 'Unknown');
    const currentTempName = t.official?.tempName || t.current?.businessName;
    if (currentTempName && currentTempName !== lastRecordedBiz && currentTempName !== '-' && currentTempName !== b.businessName) {
      if (last && !last.endedAt) last.endedAt = t.official?.lastSyncedAt || t.updatedAt || now;
      t.assignmentHistory.push({
        businessName: currentTempName,
        address: t.current?.address || t.official?.address || '',
        city: t.current?.city || t.official?.city || '',
        paymentAmount: t.current?.paymentAmount || t.official?.wishAmount || 0,
        assignedAt: t.official?.lastSyncedAt || t.updatedAt || new Date(0),
        endedAt: now,
        note: 'Auto-detected unrecorded intermediate location',
    ...
    t.setupReason = undefined;
    await t.save();
    let job = null;
    if (agent) job = await AgentJob.create({ terminal: t._id, terminalId: t.terminalId, agent: agent._id, assignedBy: req.user._id, businessName: b.businessName, address: b.address, city: b.city, wishAmount: b.wishAmount, cashToLoad: b.cashToLoad, dueAt: new Date(b.dueAt), events: [{ status: 'assigned', note: b.note, createdBy: req.user._id }] });
    await audit(req, 'TERMINAL_ASSIGNED', 'Terminal', t.id, { terminalId: t.terminalId, to: b.businessName, agent: agent?.email, cashToLoad: b.cashToLoad, wishAmount: b.wishAmount, jobId: job?.id });
    res.json({ terminal: t, job });
  } catch (e) { next(e) }
});
...
app.get('/api/jobs/active-terminal/:terminalId', auth, async (req, res, next) => {
  try {
    const jobQuery = { terminalId: req.params.terminalId.toUpperCase(), status: { $nin: ['approved', 'cancelled'] } };
    if (req.query.date) {
      const dateStr = String(req.query.date).split('T')[0];
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
        const start = new Date(dateStr + 'T00:00:00.000Z');
        ...
        jobQuery.dueAt = { $gte: start, $lt: end };
      }
    }
    const job = await AgentJob.findOne(jobQuery).populate('agent', 'name email').select('terminalId agent status dueAt businessName');
    res.json({ busy: Boolean(job), job });
  } catch (e) { next(e) }
});
...
app.patch('/api/route-sheet/:jobId', auth, permit('admin', 'manager', 'agent'), async (req, res, next) => { try { const job = await AgentJob.findById(req.params.jobId); if (!job) return res.status(404).json({ message: 'Job not found' }); if (req.user.role === 'agent' && String(job.agent) !== String(req.user._id)) return res.status(403).json({ message: 'Not authorized to edit this job' }); const b = req.body; if (b.routeExistingCash !== undefined) job.routeExistingCash = b.routeExistingCash === null ? undefined : Number(b.routeExistingCash); if (b.routeCashLoaded !== undefined) job.routeCashLoaded = b.routeCashLoaded === null ? undefined : Number(b.routeCashLoaded); if (b.routeLoadTime !== undefined) job.routeLoadTime = b.routeLoadTime === null ? undefined : String(b.routeLoadTime); if (b.routeBillsRemaining !== undefined) job.routeBillsRemaining = b.routeBillsRemaining === null ? undefined : Number(b.routeBillsRemaining); if (b.routeCashToLoad !== undefined) job.routeCashToLoad = b.routeCashToLoad === null ? undefined : Number(b.routeCashToLoad); await job.save(); res.json(job); } catch (e) { next(e) } });
app.delete('/api/route-sheet/:jobId', auth, permit('admin', 'manager'), async (req, res, next) => { try { const job = await AgentJob.findById(req.params.jobId); if (!job) return res.status(404).json({ message: 'Job not found' }); await AgentJob.deleteOne({ _id: job._id }); await audit(req, 'AGENT_JOB_CANCELLED', 'AgentJob', job.id, { terminalId: job.terminalId, agentId: job.agent }); res.json({ message: 'Job cancelled successfully' }); } catch (e) { next(e) } });
app.patch('/api/route-sheet/:jobId/reassign', auth, permit('admin', 'manager'), async (req, res, next) => { try { const job = await AgentJob.findById(req.params.jobId); if (!job) return res.status(404).json({ message: 'Job not found' }); const agent = await User.findOne({ _id: req.body.agentId, role: 'agent', active: true }); if (!agent) return res.status(400).json({ message: 'Target agent not found or inactive' }); job.agent = agent._id; job.events.push({ status: 'assigned', note: 'Reassigned by admin/manager', createdBy: req.user._id }); await job.save(); await audit(req, 'AGENT_JOB_REASSIGNED', 'AgentJob', job.id, { terminalId: job.terminalId, newAgentId: agent._id, newAgentName: agent.name }); res.json({ message: 'Job reassigned successfully' }); } catch (e) { next(e) } });
app.get('/api/location-areas', auth, async (req, res, next) => {
  try {
    const areas = await Terminal.aggregate([
      { $match: { 'official.sourcePresent': true, 'official.locationArea': { $nin: [null, ''] } } },
    ...
    mapped.push({ name: 'Inactive', terminals: inactiveCount, cities: [] });
    res.json(mapped);
  } catch (e) { next(e) }
});
app.get('/api/location-areas/:area/terminals', auth, async (req, res, next) => {
  try {
    const areaParam = String(req.params.area);
    const areaList = areaParam.split(',').map(a => a.trim()).filter(Boolean);
    const hasNeverComm = areaList.includes('Never Communicated');
    const hasInactive = areaList.includes('Inactive');
    const actualAreas = areaList.filter(a => a !== 'Never Communicated' && a !== 'Inactive');
    
    const conditions = [];
    if (actualAreas.length > 0) {
      conditions.push({ 'official.locationArea': { $in: actualAreas }, 'official.sourcePresent': true });
    }
    if (hasNeverComm) {
      conditions.push({ 'official.lastCommunication': 'Never Communicated' });
    }
    if (hasInactive) {
      conditions.push({ 'official.status': 'Inactive' });
    }

    const areaMatch = {};
    if (conditions.length > 0) {
      if (conditions.length === 1) {
        Object.assign(areaMatch, conditions[0]);
      } else {
        areaMatch.$or = conditions;
      }
    } else {
      areaMatch['official.locationArea'] = { $in: [] };
    }
    const terminals = await Terminal.find(areaMatch).select('terminalId official.status official.tempName official.name official.locationArea official.wishAmount official.cashBalance official.lastWithdrawalAt official.lastCommunication official.city current.businessName current.address current.city').sort({ 'official.locationArea': 1, 'official.city': 1, terminalId: 1 }).lean();
    ...
    const active = await AgentJob.find(jobQuery).select('terminalId agent status dueAt').populate('agent', 'name').lean();
    const jobs = new Map(active.map(j => [j.terminalId, j]));
    res.json(terminals.map(t => ({ ...t, current: { ...t.current, city: t.current?.city || t.official?.city || '' }, requiredCash: Math.max(0, (t.official?.wishAmount || 0) - (t.official?.cashBalance || 0)), activeJob: jobs.get(t.terminalId) || null })));
  } catch (e) { next(e) }
});
app.post('/api/jobs/dispatch', auth, permit('admin', 'manager', 'dispatch'), async (req, res, next) => {
  try {
    const b = z.object({ terminalId: z.string().min(2), agentId: z.string().min(2), cashToLoad: z.number().nonnegative(), dueAt: z.string().min(1), note: z.string().max(2000).optional(), localDate: z.string().optional() }).parse(req.body); const [terminal, agent] = await Promise.all([Terminal.findOne({ terminalId: b.terminalId.toUpperCase() }), User.findOne({ _id: b.agentId, role: 'agent', active: true })]); if (!terminal) return res.status(404).json({ message: 'Terminal not found' }); if (terminal.official?.status === 'Inactive') return res.status(400).json({ message: `Cannot dispatch job: Terminal ${terminal.terminalId} is currently Inactive. Please activate the terminal in Terminal Registry first.` }); if (!agent) return res.status(400).json({ message: 'Selected agent is unavailable' }); const lockQuery = { terminalId: terminal.terminalId, status: { $nin: ['approved', 'cancelled'] } };
    const dateStr = String(b.dueAt).split('T')[0];
    if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
      const start = new Date(dateStr + 'T00:00:00.000Z');
      const end = new Date(start);
      end.setUTCDate(end.getUTCDate() + 1);
      lockQuery.dueAt = { $gte: start, $lt: end };
    }
    const terminalJob = await AgentJob.findOne(lockQuery).populate('agent', 'name');
    if (terminalJob) return res.status(409).json({ message: `${terminal.terminalId} is already assigned on ${dateStr} to ${terminalJob.agent?.name || 'an agent'}. Complete and approve that job first, or select another date.` });
    if (b.cashToLoad > 0) {
      let today, tomorrow;
      ...
      const dqPrev = { date: { $lt: today } }; const jqPrev = { createdAt: { $lt: today }, status: { $nin: ['cancelled'] } };
      const dq = { date: { $gte: today, $lt: tomorrow } }; const jq = { createdAt: { $gte: today, $lt: tomorrow }, status: { $nin: ['cancelled'] } };
      const [prevWithdrawn, prevDispatched, prevReturned, withdrawn, alreadyDispatched, returned] = await Promise.all([
...
app.post('/api/jobs/dispatch-area', auth, permit('admin', 'manager', 'area'), async (req, res, next) => {
  try {
    ...
    const locked = await AgentJob.distinct('terminalId', lockQuery);
    const lockedSet = new Set(locked);
    const eligible = terminals.filter(t => !lockedSet.has(t.terminalId));
    if (!eligible.length) return res.status(409).json({ message: 'All selected ATMs already have active jobs on this date' });
    // ── Balance check ──────────────────────────────────────────────────────────
...
app.get('/api/cash/available', auth, permit('admin', 'manager', 'ledger', 'dispatch', 'area'), async (req, res, next) => {
  try {
...
// ── Cash Ledger summary ──────────────────────────────────────────────────────
app.get('/api/cash/ledger', auth, permit('admin', 'manager', 'ledger'), async (req, res, next) => {
  try {
...
// ── Cash Discrepancies ───────────────────────────────────────────────────────
app.get('/api/discrepancies', auth, permit('admin', 'manager', 'discrepancies'), async (req, res, next) => {
  try {
    const q = {};
...
app.patch('/api/discrepancies/:id', auth, permit('admin', 'manager', 'discrepancies'), async (req, res, next) => {
  try {
...
app.get('/api/logs', auth, permit('admin', 'manager', 'logs'), async (req, res, next) => {
  try {
...
app.get('/api/assignment-history', auth, permit('admin', 'history'), async (req, res, next) => {
  try {
    const escape = s => String(s || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const limit = Math.min(5000, Math.max(1, +req.query.limit || 500));
    
    const match = {};
    if (req.query.search) {
      const rx = new RegExp(escape(req.query.search), 'i');
      match.$or = [
        { terminalId: rx },
        { 'assignmentHistory.businessName': rx },
        { 'assignmentHistory.address': rx },
        { 'assignmentHistory.city': rx },
        { 'original.businessName': rx },
        { 'original.address': rx },
        { 'original.city': rx },
        { 'current.businessName': rx },
        { 'current.address': rx },
        { 'current.city': rx },
        { 'official.tempName': rx },
        { 'official.name': rx }
      ];
    }

    const terminals = await Terminal.find(match).populate('assignmentHistory.assignedBy', 'name email').lean();
    
    let items = [];
    for (const t of terminals) {
      const origBiz = t.original?.businessName || t.official?.name || t.official?.tempName || t.current?.businessName || 'Unknown';
      const origAddr = t.original?.address || t.official?.address || t.current?.address || 'Unknown';
      const origCity = t.original?.city || t.official?.city || t.current?.city || 'Unknown';
      ...
      const initialEntry = {
        ...
        originalBusiness: origBiz,
        originalAddress: origAddr,
        businessName: origBiz,
        address: origAddr,
        city: origCity,
        paymentAmount: 0,
        assignedAt: t.createdAt || new Date(0),
        endedAt: hasHistory ? t.assignmentHistory[0].assignedAt : null,
        note: 'Original Import / Seed',
        assignedBy: 'System',
        assignedByEmail: ''
      };
      terminalEntries.push(initialEntry);
      ...
      if (hasHistory) {
        for (const h of t.assignmentHistory) {
          terminalEntries.push({
            ...
            address: h.address,
            city: h.city,
            paymentAmount: h.paymentAmount || 0,
            assignedAt: h.assignedAt,
            endedAt: h.endedAt,
            note: h.note,
            assignedBy: h.assignedBy?.name || 'Unknown',
            assignedByEmail: h.assignedBy?.email || ''
          });
        }
      }
      ...
      const currentTempName = t.official?.tempName || t.current?.businessName;
      if (currentTempName && currentTempName !== lastEntry.businessName && currentTempName !== origBiz && currentTempName !== '-') {
         lastEntry.endedAt = t.official?.lastSyncedAt || t.updatedAt || new Date();
         ...
         terminalEntries.push({
            ...
            originalAddress: origAddr,
            businessName: currentTempName,
            address: t.current?.address || t.official?.address || origAddr,
            city: t.current?.city || t.official?.city || origCity,
            paymentAmount: t.current?.paymentAmount || t.official?.wishAmount || 0,
            assignedAt: t.official?.lastSyncedAt || t.updatedAt || new Date(),
            endedAt: null,
            note: 'Auto-detected from unrecorded Temp Name change',
            assignedBy: 'System',
            assignedByEmail: ''
         });
      }
      
      items.push(...terminalEntries);
    }
    
    if (req.query.city) {
      const crx = new RegExp(`^${escape(req.query.city)}$`, 'i');
      items = items.filter(i => crx.test(i.city || ''));
    }
    
    if (req.query.from) {
      const fromD = new Date(`${req.query.from}T00:00:00`);
      items = items.filter(i => new Date(i.assignedAt) >= fromD);
    }
    if (req.query.to) {
      const toD = new Date(`${req.query.to}T23:59:59.999`);
      items = items.filter(i => new Date(i.assignedAt) <= toD);
    }

    items.sort((a, b) => new Date(b.assignedAt || 0) - new Date(a.assignedAt || 0));
    
    const limitedItems = items.slice(0, limit);
    res.json({ items: limitedItems, total: items.length, limited: items.length > limit });
  } catch (e) { next(e); }
});
// ── Tickets ─────────────────────────────────────────────────────────────────
