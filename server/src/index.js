...
app.get('/api/users/agents',auth,permit('admin','manager','agents'),async(req,res,next)=>{try{
...
app.get('/api/jobs/active-terminal/:terminalId',auth,async(req,res,next)=>{try{
...
app.get('/api/location-areas',auth,async(req,res,next)=>{try{const areas=await Terminal.aggregate([{$match:{'official.sourcePresent':true}},{$group:{_id:{$cond:[{$or:[{$eq:['$official.locationArea',null]},{$eq:['$official.locationArea','']}]},'Miscellaneous','$official.locationArea']},terminals:{$sum:1},cities:{$addToSet:{$ifNull:['$official.city','$current.city']}}}},{$sort:{_id:1}}]);const mapped = areas.map(a=>({name:a._id,terminals:a.terminals,cities:a.cities.filter(Boolean)}));const misc = mapped.find(m=>m.name==='Miscellaneous');const rest = mapped.filter(m=>m.name!=='Miscellaneous');if(misc)rest.push(misc);res.json(rest);}catch(e){next(e)}});
app.get('/api/location-areas/:area/terminals',auth,async(req,res,next)=>{try{
  const areaParam=String(req.params.area);
  const areaList=areaParam.split(',').map(a=>a.trim()).filter(Boolean);
  const hasMisc = areaList.includes('Miscellaneous');
  const actualAreas = areaList.filter(a => a !== 'Miscellaneous');
  const areaMatch = {};
  if (hasMisc && actualAreas.length > 0) {
    areaMatch.$or = [{ 'official.locationArea': { $in: actualAreas } }, { 'official.locationArea': { $in: [null, ''] } }, { 'official.locationArea': { $exists: false } }];
  } else if (hasMisc) {
    areaMatch.$or = [{ 'official.locationArea': { $in: [null, ''] } }, { 'official.locationArea': { $exists: false } }];
  } else {
