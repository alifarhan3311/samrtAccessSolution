...
app.get('/api/users/agents',auth,permit('admin','manager'),async(req,res,next)=>{try{const showAll=req.query.all==='1'&&req.user.role==='admin';const filter=showAll?{role:'agent'}:{role:'agent',active:true};const agents=await User.find(filter).select('name email phoneNumber profilePicture active').sort({name:1}).lean();const workloads=await AgentJob.aggregate([{$match:{status:{$nin:['approved','cancelled']}}},{$group:{_id:'$agent',openJobs:{$sum:1}}}]);const counts=new Map(workloads.map(w=>[String(w._id),w.openJobs]));res.json(agents.map(a=>({...a,available:true,openJobs:counts.get(String(a._id))||0})));}catch(e){next(e)}});
...
app.get('/api/dashboard',auth,async(req,res,next)=>{try{
  ...
  const [total, active, inactive, inventory, alerts, setupRequired, cities,
    ...
    AgentJob.aggregate([{$match:{status:'approved',approvedAt:{$gte:today}}},{$unwind:'$events'},{$match:{'events.status':'cash_loaded'}},{$group:{_id:null,total:{$sum:'$events.cashLoaded'}}}]).then(r=>r[0]?.total||0),
    AgentJob.aggregate([{$match:{status:'approved',approvedAt:{$gte:monthStart}}},{$unwind:'$events'},{$match:{'events.status':'cash_loaded'}},{$group:{_id:null,total:{$sum:'$events.cashLoaded'}}}]).then(r=>r[0]?.total||0),
    // cash returned
  ...
  res.json({
    ...
    cash:{
      today:{ withdrawn:todayWithdrawals.total, dispatched:todayDispatched.total, actualLoaded:todayActualLoaded, returned:todayReturned.total,
               balance: todayWithdrawals.total - todayDispatched.total + todayReturned.total },
      month:{ withdrawn:monthWithdrawals.total, dispatched:monthDispatched.total, actualLoaded:monthActualLoaded, returned:monthReturned.total,
               balance: monthWithdrawals.total - monthDispatched.total + monthReturned.total },
    },
...
const batchId=crypto.randomUUID();const primaryArea=areas.join(', ');const jobs=await AgentJob.insertMany(eligible.map(t=>{const defaultLoad=Math.max(0,(t.official?.wishAmount||0)-(t.official?.cashBalance||0));const cashToLoad=b.cashOverrides?.[t.terminalId]??defaultLoad;return{batchId,locationArea:t.official?.locationArea||primaryArea,terminal:t._id,terminalId:t.terminalId,agent:agent._id,assignedBy:req.user._id,businessName:t.current?.businessName||t.official?.tempName||t.original?.businessName,address:t.current?.address||t.original?.address,city:t.current?.city||t.original?.city,wishAmount:t.official?.wishAmount||0,cashToLoad,dueAt:new Date(b.dueAt),events:[{status:'assigned',note:`Area route: ${t.official?.locationArea||primaryArea}${b.note?` — ${b.note}`:''}`,createdBy:req.user._id}]}}));await audit(req,'AREA_ROUTE_DISPATCHED','AgentJob',batchId,{locationArea:primaryArea,agent:agent.email,assigned:jobs.length,skippedLocked:locked.length,terminalIds:jobs.map(j=>j.terminalId)});res.status(201).json({batchId,assigned:jobs.length,skippedLocked:locked.length,totalCash:jobs.reduce((s,j)=>s+j.cashToLoad,0)});}catch(e){next(e)}});
...
app.post('/api/cash/withdraw',auth,permit('admin','manager'),async(req,res,next)=>{try{const b=z.object({amount:z.number().min(1),note:z.string().max(500).optional(),date:z.string().optional()}).parse(req.body);const rec=await CashWithdrawal.create({amount:b.amount,note:b.note,withdrawnBy:req.user._id,date:b.date?new Date(b.date):new Date()});await audit(req,'CASH_WITHDRAWN','CashWithdrawal',rec.id,{amount:b.amount});res.status(201).json(rec);}catch(e){next(e)}});
app.get('/api/cash/withdrawals',auth,permit('admin','manager'),async(req,res,next)=>{try{const page=Math.max(1,+req.query.page||1),limit=20;const q={};if(req.query.from)q.date={$gte:new Date(req.query.from+'T00:00:00')};if(req.query.to)q.date={...q.date,$lte:new Date(req.query.to+'T23:59:59.999')};const[items,total]=await Promise.all([CashWithdrawal.find(q).sort({date:-1}).skip((page-1)*limit).limit(limit).populate('withdrawnBy','name'),CashWithdrawal.countDocuments(q)]);const sum=await CashWithdrawal.aggregate([{$match:q},{$group:{_id:null,total:{$sum:'$amount'}}}]);res.json({items,total,pages:Math.ceil(total/limit),page,totalAmount:sum[0]?.total||0});}catch(e){next(e)}});
...
app.post('/api/cash/return',auth,permit('admin','manager'),async(req,res,next)=>{try{const b=z.object({agentId:z.string().min(1),amount:z.number().min(0),jobIds:z.array(z.string()).optional(),note:z.string().max(500).optional(),date:z.string().optional()}).parse(req.body);const rec=await CashReturn.create({agent:b.agentId,amount:b.amount,jobIds:b.jobIds||[],note:b.note,recordedBy:req.user._id,date:b.date?new Date(b.date):new Date()});await audit(req,'CASH_RETURNED','CashReturn',rec.id,{amount:b.amount,agentId:b.agentId});res.status(201).json(rec);}catch(e){next(e)}});
app.get('/api/cash/returns',auth,permit('admin','manager'),async(req,res,next)=>{try{const page=Math.max(1,+req.query.page||1),limit=20;const q={};if(req.query.agentId)q.agent=req.query.agentId;if(req.query.from)q.date={$gte:new Date(req.query.from+'T00:00:00')};if(req.query.to)q.date={...q.date,$lte:new Date(req.query.to+'T23:59:59.999')};const[items,total]=await Promise.all([CashReturn.find(q).sort({date:-1}).skip((page-1)*limit).limit(limit).populate('agent','name email').populate('recordedBy','name').populate('jobIds','terminalId businessName'),CashReturn.countDocuments(q)]);const sum=await CashReturn.aggregate([{$match:q},{$group:{_id:null,total:{$sum:'$amount'}}}]);res.json({items,total,pages:Math.ceil(total/limit),page,totalAmount:sum[0]?.total||0});}catch(e){next(e)}});
...
// ── Cash Ledger summary ──────────────────────────────────────────────────────
app.get('/api/cash/ledger',auth,permit('admin','manager'),async(req,res,next)=>{try{
